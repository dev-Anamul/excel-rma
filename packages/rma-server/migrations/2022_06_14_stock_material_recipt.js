function execute(){
    const count = db.stock_entry.find({"stock_entry_type": "Material Receipt"}).count();
...
function updateStockEntry(skip){
    const invoice = db.stock_entry.find({"stock_entry_type": "Material Receipt"}).skip(skip).limit(1);
