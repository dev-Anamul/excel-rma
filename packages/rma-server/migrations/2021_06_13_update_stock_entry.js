function execute(){
    const count = db.stock_entry.find({"stock_entry_type": "Material Transfer"}).count();
...
function updateStockEntry(skip){
    const invoice = db.stock_entry.find({"stock_entry_type": "Material Transfer"}).skip(skip).limit(1);
