function execute(){
    const count = db.stock_entry.find({"stock_entry_type": "Material Issue"}).count();
...
function updateStockEntry(skip){
    const invoice = db.stock_entry.find({"stock_entry_type": "Material Issue"}).skip(skip).limit(1);
