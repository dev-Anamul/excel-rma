...
export class StockLedgerAggregateService extends AggregateRoot {
  constructor(private readonly stockLedgerService: StockLedgerService, private readonly settingsService: SettingsService) {
    super();
  }

  getStockSummaryList(query: { limit: number; offset: number; sort: string; filter: any }) {
    ...
    return this.settingsService.find().pipe(
      ...
      switchMap(() => {
        return this.stockLedgerService
          ...
          .pipe(
            switchMap((filteredItems: Array<{ _id: string; item_name: any }>) => {
                ...
                concatMap(filterItem => {
                  return forkJoin({
                    inward: this.stockLedgerService.asyncAggregate([
                      ...
                      {
                        $group: { _id: '$item_name', totalInward: { $sum: '$actual_qty' } },
                      },
                    ]),
                    ...
                    switchMap((res: any) => {
                      return of({
                        ...
                        item_name: res.inward.length
                          ...
                          : filterItem._id,
                        total_inward: res.inward.length ? res.inward.find(x => x).totalInward : 0,
                        total_outward: res.outward.length ? res.outward.find(x => x).totalOutward : 0,
                      });
                    }),
                  );
                }),
