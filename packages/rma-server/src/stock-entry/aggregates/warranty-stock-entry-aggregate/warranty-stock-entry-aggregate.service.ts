...
export class WarrantyStockEntryAggregateService {
    ...
    return from(deliveryNotes).pipe(
      concatMap(singleDeliveryNote => {
        ...
        return forkJoin({
          ...
          valid: this.stockEntryPoliciesService.validateWarrantyStockEntry(warrantyPayload),
      ...
      switchMap(settings => {
        return from(deliveryNotes).pipe(
          concatMap(deliveryNote => {
            ...
            return this.makeStockEntry(deliveryNote, req, settings.find(x => x.settings).settings).pipe(
      ...
      switchMap(() => {
        return from(deliveryNotes).pipe(
          concatMap(deliveryNote => {
            return this.createStockLedger(deliveryNote, req.token, settingState);
          }),
        );
      }),
      catchError(err => {
  ...
  createStockLedger(payload: WarrantyStockEntryDto, token, settings: ServerSettings) {
      ...
      switchMap(fiscalYear => {
        ...
        return from(res.items).pipe(
          concatMap((item: any) => {
            ...
            res.stock_entry_type === STOCK_ENTRY_STATUS.returned ? (stockPayload.actual_qty = -item.qty) : (stockPayload.actual_qty = item.qty);
  ...
  makeStockEntry(deliveryNote: WarrantyStockEntryDto, req, settings) {
    const stockEntry = this.setStockEntryDefaults(deliveryNote, req, settings);
  ...
  removeStockEntry(stockEntry: WarrantyStockEntryDto, req) {
    return this.stockEntryPoliciesService.validateCancelWarrantyStockEntry(stockEntry.warrantyClaimUuid, stockEntry.items[0]?.serial_no[0]).pipe(
      switchMap(() => {
        if (
          stockEntry.items.find(item => {
            if (item.serial_no.filter(serial => serial.toUpperCase() === NON_SERIAL_ITEM)) {
              return undefined;
            }
            ...
            this.serialService.updateOne(
              { serial_no: stockEntryObject.items.find(x => x)?.serial_no },
              {
                $set: {
                  ...
                  'warranty.salesWarrantyDate': stockEntryObject.items?.find(x => x)?.warranty?.salesWarrantyDate,
                  'warranty.soldOn': stockEntryObject.items?.find(x => x)?.warranty?.soldOn,
                  sales_invoice_name: stockEntryObject.items.find(x => x).sales_invoice_name,
                  delivery_note: stockEntryObject.items.find(x => x).delivery_note,
                },
              },
            ),
          );
